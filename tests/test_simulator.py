...
def test_round_count_invariants():
    probs, n, _ = simulator.run(_ratings(), GOALS, completed={}, n=300, seed=1)
...
def test_probabilities_in_range():
    probs, _, _ = simulator.run(_ratings(), GOALS, completed={}, n=200, seed=2)
    for c in probs:
...
def test_strong_team_wins_more_than_weak():
    probs, _, _ = simulator.run(_ratings(strong="BRA", weak="HAI"), GOALS, completed={}, n=400, seed=3)
...
def test_completed_result_is_respected():
    ...
    probs, _, _ = simulator.run(_ratings(), GOALS, completed=completed, n=200, seed=4)
